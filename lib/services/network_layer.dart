...
class NetworkLayer {
  Future<dynamic> makeRequest(
    ...
    try {
      final response = isPost
          ? await http.post(Uri.http(BASE_URL, requestURL),
  ...
  Future<dynamic> refreshToken(String refreshToken, String language) async {
    ...
    try {
      final response = await http.post(Uri.http(BASE_URL, requestURL),
          body: {'refreshToken': refreshToken});
      if (response.statusCode == 200) {
        final Map<String, dynamic> answerMap = responseToMap(response);
        if (answerMap.containsKey('access_token') &&
            answerMap.containsKey('refresh_token')) {
          return answerMap;
        } else {
          return null;
        }
