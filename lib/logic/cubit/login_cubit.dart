...
class LoginCubit extends Cubit<LoginState> {
  ...
  void loginFunc(String smsCode, String language) async {
    if (state is LoginSms) {
      ...
      userCubit
          .makeRequest(
              url: currentState.isPhone
                  ? LOGIN_NOTIFY_VERIFY
                  : LOGIN_EMAIL_VERIFY,
              language: language,
              isPost: true,
              body: currentState.isPhone
                  ? {
        ...
        if (response is Response) {
          ...
          final newUser = UserModel(answerMap['access_token'],
              answerMap['refresh_token'], 'firebase');
          ...
          if (!currentState.isPhone) {
            ...
            if (notifStatus) {
              ...
              userCubit.makeRequest(
                  url: NOTIFICATION_URL,
                  language: language,
                  headers: {
                    'X-Auth': answerMap['access_token'],
                    'Accept': 'application/json',
                  },
                  isPost: true,
                  body: {'firebase_token': notifStatus ? token : ''});
            }
          }
        } else if (response is String) {
          emit(LoginSms(
              isPhone: currentState.isPhone,
              firstText: currentState.firstText,
              iinText: currentState.iinText,
          ...
          emit(LoginInitial(
              isPhone: currentState.isPhone,
              errorMessage:
                  language == 'kk' ? 'Белгісіз қате' : 'Неизвестная ошибка'));
        }
      });
    }
  }
    ...
    if (!notifStatus && isPhone) {
      emit(LoginInitial(
          ...
          errorMessage: language == 'kk'
              ? 'Хабарламаға рұқсат беріңіз'
              : 'Дайте разрешение на уведомление'));
    } else {
      String token = await FirebaseService().getToken();
      log(token);
      userCubit
          .makeRequest(
              url: isPhone ? LOGIN_NOTIFY_URL : LOGIN_EMAIL_URL,
              isPost: true,
              body: isPhone
                  ? {
                      'phone': '7$firstText',
                      'iin': iin,
                      'firebase_token': notifStatus ? token : ''
                    }
                  : {'email': firstText, 'iin': iin},
              language: language)
          .then((response) {
        if (response is Response) {
